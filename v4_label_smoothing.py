# ============================================================
# v4 — Label Smoothing (v2 대비 변경)
#
# 변경사항 (from v2):
#   - CrossEntropyLoss(label_smoothing=0.1) 적용
#     - 257개 클래스에서 과도한 확신 방지
#     - 모양/색상이 유사한 클래스 간 일반화 향상
#
# 근거:
#   - v3(batch=128)이 v2(batch=64)보다 낮음 → batch_size는 64가 최적
#   - v2 valid_acc=0.944이지만 모델이 test에서 overconfident할 가능성
#   - label_smoothing은 soft label로 만들어 generalization 개선
#
# Submission: Data/submissions/submission_v4_label_smoothing.csv
# ============================================================
...
if __name__ == "__main__":
    ...
    batch_size = 64   # v2와 동일 (v3 결과: batch 늘리면 오히려 손해)
    ...
    best_acc  = 0
    _exp_name = "fruit_classification_v4"
    print(f"v4 | label_smoothing=0.1 | Device={device}")
    ...
    plt.suptitle("v4 — Label Smoothing (0.1)", fontsize=13)
    plt.tight_layout()
    plt.savefig("v4_training_curve.png", dpi=150); plt.close()
    ...
    df.to_csv(submission_dir / "submission_v4_label_smoothing.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v4_label_smoothing.csv'}")

    ctypes.windll.user32.MessageBoxW(0, "v4 Done! label_smoothing=0.1", "Done", 1)
