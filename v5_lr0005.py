# v5 — max_lr 0.01 → 0.005 (낮은 lr, v2 대비 변경)
# Submission: submission_v5_lr0005.csv
...
if __name__ == "__main__":
    ...
    # ── 변경: max_lr = 0.005 ──
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.005, steps_per_epoch=len(train_loader), epochs=n_epochs)
    best_acc  = 0; _exp_name = "fruit_classification_v5"
    print(f"v5 | max_lr=0.005 | Device={device}")
    ...
    plt.suptitle("v5 — max_lr=0.005", fontsize=13); plt.tight_layout()
    ...
    df.to_csv(submission_dir / "submission_v5_lr0005.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v5_lr0005.csv'}")
    ctypes.windll.user32.MessageBoxW(0, "v5 Done! max_lr=0.005", "Done", 1)
