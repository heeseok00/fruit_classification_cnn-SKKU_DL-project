# v9 — Adam → AdamW + weight_decay=1e-4 (v2 대비 변경)
# Submission: submission_v9_adamw.csv
...
if __name__ == "__main__":
    ...
    best_acc  = 0; _exp_name = "fruit_classification_v9"
    print(f"v9 | AdamW weight_decay=1e-4 | Device={device}")
    ...
    plt.suptitle("v9 — AdamW + weight_decay=1e-4", fontsize=13); plt.tight_layout()
    ...
    df.to_csv(submission_dir / "submission_v9_adamw.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v9_adamw.csv'}")
    ctypes.windll.user32.MessageBoxW(0, "v9 Done! AdamW weight_decay=1e-4", "Done", 1)
