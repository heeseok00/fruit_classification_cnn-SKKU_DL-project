# ============================================================
# v8 — Larger Image Size (v5 대비 변경)
#
# 변경사항 (from v5):
#   - 이미지 크기: 128×128 → 160×160
#   - 나머지(AdamW, label_smoothing, batch=64, max_lr=0.01) v5와 동일
#
# 근거:
#   - 원본 이미지가 100×100 → 128×128은 적당히 업스케일
#   - 160×160으로 더 키우면 더 세밀한 시각적 특징 학습 가능
#   - 모델(GAP 사용)은 입력 크기에 무관하게 동작
#   - 단, 연산량 증가 → 학습 시간 약간 증가 예상
#
# Submission: Data/submissions/submission_v8_larger_image.csv
# ============================================================
...
if __name__ == "__main__":
    ...
    best_acc  = 0
    _exp_name = "fruit_classification_v8"
    print(f"v8 | Image=160x160 + AdamW + label_smoothing | Device={device}")
    ...
    plt.suptitle("v8 — Larger Image 160x160", fontsize=13)
    plt.tight_layout()
    plt.savefig("v8_training_curve.png", dpi=150); plt.close()
    ...
    df.to_csv(submission_dir / "submission_v8_larger_image.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v8_larger_image.csv'}")

    ctypes.windll.user32.MessageBoxW(0, "v8 Done! Image=160x160", "Done", 1)
