# v4 — batch_size 64 → 256 (v2 대비 변경)
# Submission: submission_v4_batch256.csv
...
if __name__ == "__main__":
    ...
    best_acc  = 0; _exp_name = "fruit_classification_v4"
    print(f"v4 | batch_size={batch_size} | Device={device}")
    ...
    plt.suptitle("v4 — batch_size=256", fontsize=13); plt.tight_layout()
    ...
    df.to_csv(submission_dir / "submission_v4_batch256.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v4_batch256.csv'}")
    ctypes.windll.user32.MessageBoxW(0, "v4 Done! batch_size=256", "Done", 1)
