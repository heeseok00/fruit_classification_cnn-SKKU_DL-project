# ============================================================
# v5 — AdamW + weight_decay (v4 대비 변경)
#
# 변경사항 (from v4):
#   - Adam → AdamW(lr=0.0003, weight_decay=1e-4)
#     - weight decay를 gradient 업데이트와 분리하여 독립적으로 적용
#     - Adam의 weight_decay는 gradient에 섞여 정규화 효과가 약함
#     - AdamW는 파라미터에 직접 패널티 → 일반화 향상 기대
#
# 근거:
#   - v4(label_smoothing=0.1): valid 0.950 / Kaggle 0.95502 (best)
#   - train 0.530 << valid 0.950 → underfitting, 모델 일반화 여지 있음
#   - AdamW는 label_smoothing과 독립적으로 정규화 추가 가능
#
# Submission: Data/submissions/submission_v5_adamw.csv
# ============================================================
...
class Classifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.stage1 = nn.Sequential(ResBlock(64, 64), ResBlock(64, 128, stride=2))
        self.stage2 = nn.Sequential(ResBlock(128, 128), ResBlock(128, 256, stride=2))
        self.stage3 = nn.Sequential(ResBlock(256, 256), ResBlock(256, 512, stride=2))
        self.stage4 = nn.Sequential(ResBlock(512, 512), ResBlock(512, 512))
...
if __name__ == "__main__":
    ...
    print(f"Dataset: {_dataset_dir} | Classes: {num_classes}")
    ...
    # ── 변경: Adam → AdamW(weight_decay=1e-4) ──
    ...
    best_acc  = 0
    _exp_name = "fruit_classification_v5"
    print(f"v5 | AdamW(weight_decay=1e-4) + label_smoothing=0.1 | Device={device}")
    ...
    for epoch in range(n_epochs):
        ...
        for imgs, labels in tqdm(train_loader, desc=f"[Train {epoch+1}/{n_epochs}]"):
            ...
            train_accs.append((logits.argmax(-1) == labels.to(device)).float().mean().item())
        ...
        for imgs, labels in tqdm(valid_loader, desc=f"[Valid {epoch+1}/{n_epochs}]"):
            ...
            valid_accs.append((logits.argmax(-1) == labels.to(device)).float().mean().item())
    ...
    plt.suptitle("v5 — AdamW (weight_decay=1e-4) + label_smoothing=0.1", fontsize=13)
    plt.tight_layout()
    plt.savefig("v5_training_curve.png", dpi=150); plt.close()
    ...
    with torch.no_grad():
        for data, _, file_id in tqdm(test_loader, desc="Testing"):
            pred = np.argmax(model_best(data.to(device)).cpu().numpy(), axis=1)
            prediction += pred.tolist(); file_ids += list(file_id)

    submission_dir = Path("./Data/submissions")
    submission_dir.mkdir(parents=True, exist_ok=True)
    df = pd.DataFrame({"ID": file_ids, "Category": prediction})
    df.to_csv(submission_dir / "submission_v5_adamw.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v5_adamw.csv'}")

    ctypes.windll.user32.MessageBoxW(0, "v5 Done! AdamW weight_decay=1e-4", "Done", 1)
