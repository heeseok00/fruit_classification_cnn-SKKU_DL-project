# v8 — GAP 이후 Dropout(0.3) 추가 (v2 대비 변경)
# Submission: submission_v8_dropout.csv
...
if __name__ == "__main__":
    ...
    best_acc  = 0; _exp_name = "fruit_classification_v8"
    print(f"v8 | Dropout(0.3) | Device={device}")
    ...
    plt.suptitle("v8 — Dropout(0.3)", fontsize=13); plt.tight_layout()
    ...
    df.to_csv(submission_dir / "submission_v8_dropout.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v8_dropout.csv'}")
    ctypes.windll.user32.MessageBoxW(0, "v8 Done! Dropout(0.3)", "Done", 1)
