# v6 — max_lr 0.01 → 0.03 (높은 lr, v2 대비 변경)
# Submission: submission_v6_lr003.csv
...
if __name__ == "__main__":
    ...
    # ── 변경: max_lr = 0.03 ──
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.03, steps_per_epoch=len(train_loader), epochs=n_epochs)
    best_acc  = 0; _exp_name = "fruit_classification_v6"
    print(f"v6 | max_lr=0.03 | Device={device}")
    ...
    plt.suptitle("v6 — max_lr=0.03", fontsize=13); plt.tight_layout()
    ...
    df.to_csv(submission_dir / "submission_v6_lr003.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v6_lr003.csv'}")
    ctypes.windll.user32.MessageBoxW(0, "v6 Done! max_lr=0.03", "Done", 1)
