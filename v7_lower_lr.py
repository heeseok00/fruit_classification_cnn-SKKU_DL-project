# ============================================================
# v7 — Lower max_lr (v5 대비 변경)
#
# 변경사항 (from v5):
#   - OneCycleLR max_lr: 0.01 → 0.005
#   - 나머지(AdamW, label_smoothing, batch=64, augmentation) v5와 동일
#
# 근거:
#   - max_lr=0.01이 너무 커서 1-epoch 내 수렴이 불안정할 수 있음
#   - max_lr을 절반으로 줄여 더 안정적인 수렴 시도
#   - AdamW + label_smoothing 조합에서 최적 lr 탐색
#
# Submission: Data/submissions/submission_v7_lower_lr.csv
# ============================================================
...
if __name__ == "__main__":
    ...
    # ── 변경: max_lr 0.01 → 0.005 ──
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, max_lr=0.005,
        steps_per_epoch=len(train_loader), epochs=n_epochs,
    )
    best_acc  = 0
    _exp_name = "fruit_classification_v7"
    print(f"v7 | max_lr=0.005 + AdamW + label_smoothing | Device={device}")
    ...
    plt.suptitle("v7 — Lower max_lr=0.005", fontsize=13)
    plt.tight_layout()
    ...
    df.to_csv(submission_dir / "submission_v7_lower_lr.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v7_lower_lr.csv'}")

    ctypes.windll.user32.MessageBoxW(0, "v7 Done! max_lr=0.005", "Done", 1)
