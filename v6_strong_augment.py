# ============================================================
# v6 — Stronger Augmentation (v5 대비 변경)
#
# 변경사항 (from v5):
#   - train_tfm에 augmentation 추가:
#       + RandomPerspective(distortion_scale=0.2, p=0.5)
#       + RandomGrayscale(p=0.1)
#   - 나머지(AdamW, label_smoothing, batch=64, max_lr=0.01) v5와 동일
#
# 근거:
#   - 1-epoch 학습에서 데이터 다양성이 일반화의 핵심
#   - RandomPerspective: 과일의 다양한 촬영 각도 시뮬레이션
#   - RandomGrayscale: 색상에 과도하게 의존하지 않도록 유도
#
# Submission: Data/submissions/submission_v6_strong_augment.csv
# ============================================================
...
if __name__ == "__main__":
    ...
    best_acc  = 0
    _exp_name = "fruit_classification_v6"
    print(f"v6 | StrongAugment + AdamW + label_smoothing | Device={device}")
    ...
    plt.suptitle("v6 — Stronger Augmentation", fontsize=13)
    plt.tight_layout()
    plt.savefig("v6_training_curve.png", dpi=150); plt.close()
    ...
    df.to_csv(submission_dir / "submission_v6_strong_augment.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v6_strong_augment.csv'}")

    ctypes.windll.user32.MessageBoxW(0, "v6 Done! StrongAugment (Perspective+Grayscale)", "Done", 1)
