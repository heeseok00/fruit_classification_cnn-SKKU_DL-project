# ============================================================
# v9 — Deeper Model: stage5 추가 (v5 대비 변경)
#
# 변경사항 (from v5):
#   - Classifier에 stage5 추가: ResBlock(512, 512) × 2
#   - 모델 깊이 증가 → 더 복잡한 특징 학습 가능
#   - 나머지(AdamW, label_smoothing, batch=64, max_lr=0.01, 128×128) v5와 동일
#
# 근거:
#   - v5까지 모든 실험이 0.955 근방에서 정체
#   - 모델 용량 자체가 bottleneck일 가능성
#   - GAP 덕분에 stage 추가해도 FC layer 크기 변화 없음
#
# Submission: Data/submissions/submission_v9_deeper_model.csv
# ============================================================
...
class Classifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.stage5 = nn.Sequential(ResBlock(512, 512), ResBlock(512, 512))  # 추가
...
if __name__ == "__main__":
    ...
    best_acc  = 0
    _exp_name = "fruit_classification_v9"
    print(f"v9 | Deeper(stage5) + AdamW + label_smoothing | Device={device}")
    ...
    plt.suptitle("v9 — Deeper Model (stage5 added)", fontsize=13)
    plt.tight_layout()
    plt.savefig("v9_training_curve.png", dpi=150); plt.close()
    ...
    df.to_csv(submission_dir / "submission_v9_deeper_model.csv", index=False)
    print(f"Saved: {submission_dir / 'submission_v9_deeper_model.csv'}")
    print("Done!")
